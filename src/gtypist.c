            ...
            if (cl_args.word_processor_flag && rc == ASCII_SPACE &&
              ...
              if (chars_in_the_line_typed < COLS)
              {
                wideaddch_rev( *widep == ASCII_NL ? DRILL_NL_ERR :
                               ...
                               (cl_args.show_errors_flag ? rc : DRILL_CH_ERR)));
                ++chars_in_the_line_typed;
              }
...
*/
static
void parse_file( FILE *script, char *label, long offset )
{
...
const char *get_last_lesson_filename()
{
  ...
  {
    // calculate file name
    filename = (char *)malloc(strlen( global_home_dir ) + strlen( LASTPRACTICE_FILENAME ) + 2 );
    if( filename == NULL )
    {
      perror( "malloc" );
      fatal_error( _( "internal error: malloc" ), NULL );
